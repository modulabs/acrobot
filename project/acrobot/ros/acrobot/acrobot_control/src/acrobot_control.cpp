...
int main(int argc, char **argv)
{
  ...
  Link link1(0.171,0.377790,0.27948,1.6084E-2,0.1);
  Link link2(0.253,0.388200,0.31992,2.9096E-2,0.1);
  Acrobot acrobot(link1,link2);

  float q1, q2, q1_dot,q2_dot,q1_ddot,q2_ddot;
  float errQ=0,errQdot=0;
...
//  ROS_INFO("q : %f %f %f %f", q1, q2, q1_dot, q2_dot);
  ...
  errQ = des_q2 - q2;
  while(abs(errQ)>pi){
      if(errQ>pi) errQ = errQ - 2*pi;
      else errQ = errQ + 2*pi;
  }
  
  errQdot = des_q2_dot - q2_dot; 
  while(abs(errQ)>pi){
      if(errQdot>pi) errQdot = errQdot - 2*pi;
      else errQdot = errQdot + 2*pi;
  }


  v2 = des_q2_ddot + kd*(des_q2_dot - q2_dot) + kp*(des_q2 - q2);


  P = B*u-C*q_dot-G;

 ROS_INFO("q1_dot : %f ",q1_dot);
 ROS_INFO("des_q2 : %f ",des_q2);
