...
float diff_state(float qd, float q)
{
...
public:
    Acrobot(const Link& _link1, const Link& _link2, ros::NodeHandle* _nh)
        :link1(_link1),link2(_link2),nh(*_nh)
    {}
...
////   H(q)q_ddot + C(q,q_dot)q_dot + G(q) = Bu
    ...
    void set_subscriber(){
    ros::Subscriber sub = nh.subscribe("/rrbot/joint_states", 1000, &Acrobot::listen_joint_state_callback, this);
    ROS_INFO("check");
   }
...
int main(int argc, char **argv)
{
  ...
  Link *link1 = new Link(1,1,0.5,0.08,0.1);
  Link *link2 = new Link(2,2,1,0.33,0.1);
  Acrobot *acrobot = new Acrobot(*link1, *link2, &n);
  acrobot->Acrobot_init();
...
//  ros::Subscriber sub = n.subscribe("/camera/depth_registered/points", 1000, boost::bind(&Acrobot::listen_joint_state, &acrobot, _1));
  ...
  v2 = des_q2_ddot + kd*(des_q2_dot - q2_dot) + kp*diff_state(des_q2, q2);
  P = B*u-C*q_dot-G;
